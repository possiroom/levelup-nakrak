...
public class MansionRoomSystem : MonoBehaviour
{
    ...
    [Header("Test Effects")]
    [SerializeField] private RoomTransitionTestEffects roomTransitionTestEffects;
    ...
    private void Awake()
    {
        ...
        if (roomTransitionTestEffects == null)
            roomTransitionTestEffects = FindFirstObjectByType<RoomTransitionTestEffects>();
    ...
    private IEnumerator EnterDoorRoutine(int fromRoomId, DoorDirection exitDirection)
    {
        ...
        // 방 이동 전 테스트 연출: 깜빡임 → 암전
        if (roomTransitionTestEffects != null)
        {
            yield return roomTransitionTestEffects.PlayBeforeRoomMove();
        }
        else
        {
        ...
        // 방 이동 후 테스트 연출: 의자 이동 + 발소리 + 암전 해제
        if (roomTransitionTestEffects != null)
            roomTransitionTestEffects.PlayAfterRoomMove(fromRoomId, targetRoomId);
    ...
    public void DeleteRoom(int roomId)
    {
        ...
        if (AIManager.Instance != null)
            AIManager.Instance.RoomDeleted(roomId);
    ...
    public bool IsAIRoomAvailable(int roomId)
    {
        return roomId != NoRoom && roomMap.ContainsKey(roomId) && !deletedRooms.Contains(roomId);
    }

    public int GetSafeAIRoomAfterDelete(int deletedRoomId)
    {
        List<int> neighbors = GetAINeighbors(deletedRoomId);
        foreach (int neighbor in neighbors)
        {
            if (IsAIRoomAvailable(neighbor))
                return neighbor;
        }

        if (IsAIRoomAvailable(MainRoomId))
            return MainRoomId;

        foreach (int roomId in roomMap.Keys)
        {
            if (IsAIRoomAvailable(roomId))
                return roomId;
        }
    ...
    public List<int> GetAINeighbors(int roomId)
    {
        List<int> neighbors = new List<int>();

        if (roomId == NoRoom || !doorMap.ContainsKey(roomId))
            return neighbors;

        RoomDoorData data = doorMap[roomId];
        AddAINeighbor(neighbors, data.upDoor);
        AddAINeighbor(neighbors, data.downDoor);
        AddAINeighbor(neighbors, data.leftDoor);
        AddAINeighbor(neighbors, data.rightDoor);

        return neighbors;
    }

    public bool MoveEnemyToRoom(GameObject enemy, int roomId)
    {
        if (enemy == null || !roomMap.ContainsKey(roomId) || deletedRooms.Contains(roomId))
            return false;

        Transform spawnPoint = roomMap[roomId].GetAnySpawnPoint();
        enemy.transform.position = spawnPoint != null ? spawnPoint.position : roomMap[roomId].transform.position;
        return true;
    }

    public RoomMoveInfo GetRoomMoveInfo(int currentRoomId, int nextRoomId)
    {
        if (!roomMap.ContainsKey(currentRoomId) || !roomMap.ContainsKey(nextRoomId))
            return null;

        if (deletedRooms.Contains(nextRoomId))
            return null;

        DoorDirection exitDirection = GetExitDirection(currentRoomId, nextRoomId);
        DoorDirection enterDirection = GetOppositeDirection(exitDirection);

        Transform doorPoint = roomMap[currentRoomId].GetDoorPoint(exitDirection);
        Transform spawnPoint = roomMap[nextRoomId].GetSpawnPoint(enterDirection);

        if (doorPoint == null || spawnPoint == null)
            return null;

        return new RoomMoveInfo(currentRoomId, nextRoomId, doorPoint, spawnPoint);
    }

    public RoomMoveInfo GetAIRoomMoveInfo(int currentRoomId, int nextRoomId)
    {
        if (!IsAIRoomAvailable(nextRoomId))
            return null;

        return GetRoomMoveInfo(currentRoomId, nextRoomId);
    }

    private void AddAINeighbor(List<int> neighbors, int roomId)
    {
        if (!IsAIRoomAvailable(roomId))
            return;

        if (!neighbors.Contains(roomId))
            neighbors.Add(roomId);
    }
    ...
    private DoorDirection GetExitDirection(int currentRoomId, int nextRoomId)
    {
        ...
        RoomDoorData data = doorMap[currentRoomId];

        if (data.upDoor == nextRoomId)
            return DoorDirection.Up;

        if (data.downDoor == nextRoomId)
            return DoorDirection.Down;

        if (data.leftDoor == nextRoomId)
            return DoorDirection.Left;

        if (data.rightDoor == nextRoomId)
            return DoorDirection.Right;
